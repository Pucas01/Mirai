...
Item {
    ...
    component ActBtn: Item {
        ...
        Canvas {
            ...
            onPaint: DivaPaint.paintFacetPill(actCanvas, parent.active ? 1.0 : Math.max(hp, 0), 5)
        }
        ...
        MouseArea {
            ...
            onPositionChanged: mouse => {
                actCanvas.mx = Math.max(0, Math.min(1, mouse.x / width))
                actCanvas.my = Math.max(0, Math.min(1, mouse.y / height))
            }
